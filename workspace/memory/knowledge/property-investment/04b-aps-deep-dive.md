# Australian Property Scout — Deep Dive (Part 2)
*Research: Feb 2026 — School of Property, podcast content, strategy framework*

---

## School of Property (schoolofproperty.com.au)

APS's education arm — a paid course/program separate from the BA service.

**Platform:** Kajabi (schoolofproperty.mykajabi.com)

### Confirmed Course Specs (from Kajabi sales page — Feb 2026)
- **11 modules** (NOT 12 — earlier "12 modules" snippet appears outdated)
- **70+ lessons**
- **50+ hours of video content**
- Lifetime access (including all new content added in future)
- Easy-to-navigate course portal
- Exclusive access to resource libraries and expert panel
- Step-by-step approach for beginners to experts

**Note:** A prior search snippet said "12 modules with sharp and practical videos" — this appears to be from an older version or cached page. The live Kajabi page as of Feb 2026 says 11 modules, 70+ lessons, 50+ hours. The "98+ lessons" figure from a prior session is unverified and likely outdated.

### Curriculum Outline (from schoolofproperty.com.au/what-we-do)
Sammy Gordon's confirmed modules — exact lesson counts unknown, but topics:

1. **Property Investing Fundamentals** — foundational knowledge base, packed with insights
2. **The Trident Formula** — Sam's three-pronged approach to building a strategic portfolio (this is APS's core framework)
3. **Advanced Accounting and Structuring** — from Sam's personal accountant; includes special offer; bullet-proof portfolio structuring
4. **Bonus Podcast Episodes** — behind-the-curtain Scouting Australia Podcast episodes (exclusive to SoP members)
5. **Advanced Investor Strategies & Concepts** — Sam + expert brokers unpack strategies he's developed over years
6. **Financial Masterclass for Investors** — steps Sam used to build a 20-property portfolio making $100k p.a.
7. **Real Deals** — Sam dissects deals from APS vault + his own portfolio
8. **Scaling Your Portfolio** — how Sam scaled to 20+ properties on average Australian income
9. **'How To' Series** — straight strategy sessions: how to build a property portfolio

*Note: 9 topics confirmed from the website. Additional modules/lessons not yet captured (11 total on Kajabi).*

**Positioning:** "The ultimate roadmap to property investing success... No fluff, no un-tested theory — just the exact blueprint that has helped thousands of everyday Australians build life-changing wealth."

**Key framework:** The Trident Formula — Sam's "three pronged approach"

### Trident Formula — Confirmed Components (Feb 2026)

The exact "Trident Formula" branding is only used internally/in the School of Property (not searchable publicly). However, Sam Gordon described his strategy explicitly in a 2019 Property Investory podcast interview:

> *"Buying in below market with strong cash flows in solid growth markets with value add potential."*

This maps directly to three prongs:
1. **Below market value** (buy at a discount — manufactured equity from day one)
2. **Strong cash flow** (yield sufficient to hold without cash drain)
3. **Capital growth + value add potential** (suburb fundamentals + ability to improve)

This is the APS "bread and butter" deal type — the three-legged stool that all APS property sourcing is filtered through. All three must be present. If only two prongs are met, it doesn't qualify.

**Why this matters for PropPath:** The Trident Formula is a deal-qualification framework, not a portfolio-stage framework. It answers "is this the right property?" — not "what stage am I at?" PropPath models the stages (Foundation/Momentum/Passive for IK; equivalent for APS clients), while the Trident Formula is the filter applied at each stage when sourcing.

**Real outcomes from School of Property clients:**
- Bailey: started with $60k equity, built 3-property portfolio in 12 months → $480k equity created
- Adam & Kerry: built $2.136M portfolio in 12 months → $530k equity unlocked

**Key episode:** "Inside the School of Property" — Sam + Head of Education Brendan Geoghegan broke down the curriculum, modules, and lessons. Released Nov 2024.

**Why it matters for PropPath:** School of Property is the education layer; PropPath could be the *ongoing execution layer* — the living dashboard that students use after they've learned the blueprint.

---

## APS Strategy Framework — Confirmed Deep Detail

### The Portfolio Blueprint Process (APS)
1. Strategy session — understand goals, experience, finances
2. Reverse engineer to income target (e.g. $100k/yr = 4 properties × $25k yield)
3. Set timeframe + purchase frequency based on borrowing capacity
4. Education-first: before sourcing, educate client on why that market/property
5. Source + negotiate (off-market network)
6. Support + reinvest — stay in touch, coordinate broker/accountant, trigger next cycle

### Key Podcast Topics (Scouting Australia — ongoing intel)
From episode chapter lists — these are the things APS clients think about:

**Portfolio building mechanics:**
- "3 properties off 1 deposit" — the signature APS outcome
- "12 properties in 3 years" — aggressive scaling case study
- Buy and hold myth busted (2024)
- When to sell vs hold

**Finance & serviceability:**
- What to do when banks say no
- SMSF strategies
- Interest rates vs inflation
- Low-risk to banks: how to position yourself

**Market selection:**
- How to pick a growth market
- Understanding data
- Investing against the herd (myth-busting)
- Blue collar vs blue chip

**Deal types:**
- Bread & butter (3-part series)
- Cash flow dead? (full episode)
- Manufactured equity (renovation focus)
- Commercial transition (advanced, larger portfolios)

**Mistakes:**
- Overconfidence after initial success
- House & land packages — "Trifecta of Idiocy" (APS strongly against OTP/H&L)
- Getting distracted by luxury spending (kills borrowing capacity)
- Not diversifying geographically
- Impatience (long-term game)
- A more expensive property isn't a better investment

**Mindset:**
- Sacrifice for long-term freedom
- 92% of Australians never own more than 1-2 properties
- Running your own race
- Community/accountability

### APS Anti-patterns (What They Tell Clients NOT to Do)
- House & land packages (OTP) — explicitly called "Trifecta of Idiocy"
- Buying locally (emotional, expensive, wrong yield)
- Luxury cars / lifestyle spending before wealth is built
- Going it alone without specialists
- Working with inexperienced advisors
- Selling too early

---

## APS Business Scale (Context for PropPath)

- 80+ full-time staff across InvestorKit Group (wait — this is InvestorKit, not APS)
- APS: team of buyer's agents + in-house renovation/construction project management team
- Sam Gordon personally: 84 individual title properties, 108 rentals, $48M+ portfolio
- Awards: REB BA of the Year 2024, Strategic Investor of Year 2020
- Summit events, community events, mentoring program (2025)
- "Mentor Clients" tier — more intensive support (Nathan & Katelyn case study)

---

## APS Content Calendar (Ongoing Research Opportunities)

| Episode/Topic | PropPath Relevance |
|---|---|
| "Equity Explained" | How equity unlock triggers next purchase — key PropPath feature |
| "Bread & Butter" 3-part series | Deal type education — what PropPath roadmap shows |
| "12 properties in 3 years" (Nick Wilcox) | Aggressive portfolio scaling — advanced user journey |
| "Is Cash Flow Dead?" | Cash flow vs growth balance — portfolio health metric |
| "What to do when banks say no" | Serviceability cap hit — PropPath should show when this happens |
| "SMSF Strategies" | Advanced structure — PropPath could show SMSF sub-portfolio |
| "Commercial Transition" | Phase 3+ portfolio — PropPath should model commercial assets too |
| "School of Property update" | Nov 2024 — curriculum expansion, new modules |
